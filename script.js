...
function objetoCelula(texto){
  ...
  const precisaVerMais = conteudo.length > 260;
  const id = "obj_" + Math.random().toString(36).substring(2, 10);

  return `
    <div id="${id}" class="objeto-resumido" title="${String(conteudo).replace(/"/g, '&quot;')}">${conteudo}</div>
    ${precisaVerMais ? `<span class="botao-ver-mais" onclick="alternarObjeto('${id}', this)">Ver mais</span>` : ""}
  `;
}
...
function alternarObjeto(id, botao){
  ...
  botao.innerText = el.classList.contains("expandido") ? "Ver menos" : "Ver mais";
}
